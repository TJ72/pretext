...
| Safari's `word-break: keep-all` punctuation boundary differs from Chromium and Gecko for no-space mixed CJK text. | `Related`: comment 4 on [WebKit #197411](https://bugs.webkit.org/show_bug.cgi?id=197411) describes Safari refusing Japanese comma/period breaks under `keep-all`. That issue is resolved as a duplicate of fixed [WebKit #235266](https://bugs.webkit.org/show_bug.cgi?id=235266), so it is not an exact open match for the current Safari behavior. | On Safari 26.4, using the Chromium/Gecko policy makes `foo。bar日本語` five lines instead of Safari's four. | Keep punctuation policy in preprocessing through `breakKeepAllAfterPunctuation`; do not move it into `layout()`. |
